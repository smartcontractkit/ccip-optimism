...
interface ProofParams {
  ...
  address: string;
  addressManagerContractAddress: string;
  targetContractABI: ethers.ContractInterface;
  node: string;
}

export const getProof = async (params: ProofParams) => {
  const { l1Provider, l2Provider, address, targetContractABI, addressManagerContractAddress, node } = params;
  const addressManager = new ethers.Contract(addressManagerContractAddress, abis.addressManagerABI, l1Provider);
...
const getLatestStateBatchHeader = async (
  ...
  const stateCommitmentChain = await loadContractFromManager(
    abis.stateCommitmentChain,
    'StateCommitmentChain',
    addressManager,
    l1Provider
  );
  for (let endBlock = await l1Provider.getBlockNumber(); endBlock > 0; endBlock = Math.max(endBlock - 100, 0)) {
